...
void do_calc(){
    ...
    //M is number of properties available
    ...
    for(int i = 0; i < S; i++){
        int li; 
        cin>>li;
        eaG.addEdge(N+M+i,end,li,0);
    }
    for(int i = 0; i <M; i++){
        int state_belonging;
        cin>>state_belonging;
        state_belonging--;
        eaG.addEdge(N+i,N+M+state_belonging,1,0);
    }
    int maxval = 10000;
    for(int i = 0; i < N; i++){
        ...
        for(int j = 0; j < M; j++){
            int bidij;
            cin>>bidij;
            eaG.addEdge(i,N+j,1,maxval-bidij);
        }
    }
   ...
   cout<<flow2<<" " << -(cost2-flow2*maxval)<<endl;
}
