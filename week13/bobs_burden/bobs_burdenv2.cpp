...
void testcases() {
	int K; 
	cin>>K;
	//k is number of layers in tree
	//then there are (k)(k+1)/2 balls
	int V = K*(K+1);
	...
	for(int i = 0; i < K; i++){
		...
		for(int j = 0; j <= i; j++){
			...
			Edge e;
			bool succ;
			tie(e,succ) = add_edge(2*(shift+j),2*(shift+j)+1,G);
			weightmap[e] = u;

			if(i > 0){
				Edge e;
				bool succ;
				tie(e,succ) = add_edge(2*(shift+j)+1,2*((prevshift)+j),G);
				weightmap[e] = 0;
			}
			if(i < K-1){
				Edge e;
				bool succ;
				tie(e,succ) = add_edge(2*(shift+j)+1,2*(shift+i+1+j),G);
				weightmap[e] = 0;
			}
			if(i > 0 && j > 0){
				Edge e;
				bool succ;
				tie(e,succ) = add_edge(2*(shift+j)+1,2*(prevshift+(j-1)),G);
				weightmap[e] = 0;
			}
			if(i < K-1 && j <i){
				Edge e;
				bool succ;
				tie(e,succ) = add_edge(2*(shift+j)+1,2*(shift+i+1+j+1),G);
				weightmap[e] = 0;
			}
			if(j > 0){
				Edge e;
				bool succ;
				tie(e,succ) = add_edge(2*(shift+j)+1,2*(shift+j-1)+1,G);
				weightmap[e] = 0;
			}
			if(j < i)
			{
				Edge e;
				bool succ;
				tie(e,succ) = add_edge(2*(shift+j)+1,2*(shift+j+1),G);
				weightmap[e] = 0;
			}
		}
		prevshift = shift;
