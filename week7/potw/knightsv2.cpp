...
void do_calc(){
	...
	for(int i = 0; i < m; i++){
		for(int j = 0; j < n; j++){
			//create the graph
			eaG.addEdge(i + j*m, m*n + i+j*m, c); // primal to dual cost c
			...
			if(i == 0 || j == 0 || i == m-1 || j == n-1){
				if(i == 0 && j == 0){
					eaG.addEdge(m*n+i+j*m,end,2); //corner has 2 exits
				}else{
					if(i == 0 && j == n-1){
