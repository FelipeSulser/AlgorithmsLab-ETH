...
void testcases() {
	...
	for(int i = 0; i < l; i++){
		int g,d;
		cin>>g>>d;
		eaG.addEdge(start,i,g); //i provide u g soldiers
		eaG.addEdge(i,sink,d); //in the end, you need to have d soldiers at least
		cumsum += d;
	}
	...
	long flow1 = push_relabel_max_flow(G, start, sink);
	if(flow1 >= cumsum){
		cout<<"yes"<<endl;
	}else{
		cout<<"no"<<endl;
